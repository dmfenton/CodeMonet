 ...
 */
function makeCodeExecutionStarted(toolName: string, iteration: number = 1): ServerMessage {
  return {
    type: 'code_execution',
    status: 'started',
...
describe('status transitions during drawing', () => {
  ...
  it('paused status overrides everything', () => {
    const startState: CanvasHookState = { ...initialState, paused: false, pieceNumber: 0 };
    const messages: ServerMessage[] = [makeCodeExecutionStarted('draw_paths', 1), makePaused(true)];
