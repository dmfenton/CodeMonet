 ...
 *
 * Run: npm run test -w web -- usePerformer
 *
 */
...
beforeEach(() => {
  ...
  global.requestAnimationFrame = vi.fn(mockRequestAnimationFrame);
  global.cancelAnimationFrame = vi.fn(mockCancelAnimationFrame);
});
...
describe('usePerformer - Stroke Animation Flow', () => {
  ...
  it('advances through all points then dispatches STROKE_COMPLETE', () => {
    ...
    const dispatch = vi.fn((action: CanvasAction) => {
      dispatchedActions.push(action);
      state = canvasReducer(state, action);
    });

    const { rerender } = renderHook((props: UsePerformerOptions) => usePerformer(props), {
      initialProps: {
  ...
  it('dispatches STAGE_COMPLETE after all strokes in batch complete', () => {
    const strokes = [makeStroke([{ x: 0, y: 0 }]), makeStroke([{ x: 10, y: 10 }])];
  ...
  it('passes stroke style on first batch only', () => {
    ...
    const progressActions = dispatchedActions.filter(
      (a): a is CanvasAction & { type: 'STROKE_PROGRESS_BATCH' } =>
        a.type === 'STROKE_PROGRESS_BATCH'
    );
...
describe('usePerformer - Edge Cases', () => {
  it('handles empty onStage gracefully (no dispatch)', () => {
    ...
    expect(dispatch).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'STROKE_COMPLETE' }));
  });
